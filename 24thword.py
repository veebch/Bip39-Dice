...
class Bip39Check(object):
    def __init__(self, language):
        ...
        if len(self.worddict) != self.radix:
            raise ValueError(
                "Expecting %d words, not %d", self.radix, len(self.worddict)
            )
    ...
    def _scan(self):
        ...
        for i in range(0, 2**entropy_to_fill):
            ...
            print(" > Binary:", binary)
            entropy_str = (entropy_candidate).to_bytes(
                (entropy_candidate.bit_length() + 7) // 8, "big"
            )
...
def main():
    ...
    if len(fullwordlist) != 23:
        print("Your list has", len(fullwordlist), "words. Exiting.")
        exit(1)
    print("---------------------")
    phrase = fullwordlist
    ...
    print(
        "You now have a valid bitcoin wallet seed. Restore it in the software of your choice"
    )
